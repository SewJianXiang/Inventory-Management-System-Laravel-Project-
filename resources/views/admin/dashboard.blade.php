...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="bg-white p-6 rounded-xl shadow-lg">
        <div class="flex items-center mb-4">
            ...
            <h2 class="text-xl font-semibold text-gray-800">Recent User Registrations</h2>
        </div>
        <div class="overflow-x-auto">
            <table class="min-w-full table-auto">
                <thead>
                    <tr class="bg-gradient-to-r from-indigo-500 to-purple-600 text-white">
                        <th class="px-3 py-3 text-left text-sm font-medium">Avatar</th>
                        <th class="px-3 py-3 text-left text-sm font-medium">Name</th>
                        <th class="px-3 py-3 text-left text-sm font-medium">Email</th>
                        <th class="px-3 py-3 text-left text-sm font-medium">Registered At</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($recentUsers as $user)
                        <tr class="border-b hover:bg-gray-50 transition-colors duration-200">
                            <td class="px-3 py-3">
                                <img src="https://ui-avatars.com/api/?name={{ urlencode($user->name) }}&size=32&background=random" class="w-8 h-8 rounded-full" alt="Avatar">
                            </td>
                            <td class="px-3 py-3 text-sm font-medium text-gray-900">{{ $user->name }}</td>
                            <td class="px-3 py-3 text-sm text-gray-600">{{ $user->email }}</td>
                            <td class="px-3 py-3 text-sm text-gray-600">{{ $user->created_at->format('M d, Y H:i') }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="px-3 py-6 text-center text-gray-500 text-sm">No recent registrations.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
